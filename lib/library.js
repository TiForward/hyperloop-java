...
function generateCustomJavaClass(options, state, metabase, fullClassname) {
	...
	Object.keys(classinfo.properties).forEach(function(name) {
		...
		if (item.metatype == 'enum') {
			enum_code.push(indent+attribute+item.metatype+' '+item.name+' {'+item.value.join(',')+'};');
...
function prepareClass(options, metabase, state, classname, code) {
	...
	var constructors = metabase.classes[classname].methods['<init>'];
	if (constructors) {
		constructors.forEach(function(m){
			var mangled = jsgen.generateMethodName(classname, 'constructor_'+m.args.length);
			addExtern(state, 'jobject '+mangled+'(JSContextRef,const JSValueRef[],JSValueRef*);');
		});
	}
...
function generateJNIClassImpl(options, metabase, state, classname, code) {
	...
	var constructors = metabase.classes[classname].methods['<init>'];
	if (constructors) {
		// collect method indices order by argument count
		var initIndices = getMethodIndicesWithSameArgumentCount(constructors);
		Object.keys(initIndices).forEach(function(argc) {
			var mangled = jsgen.generateMethodName(classname, 'constructor_'+argc);
			code.push('EXPORTAPI jobject '+mangled+'(JSContextRef ctx, const JSValueRef arguments[], JSValueRef* exception)');
			code.push('{');
			code.push('\tLOGD(\"'+mangled+'\");');
			code.push(generateJNIConstructor(options, metabase, state, code, '\t', classname, classSig, constructors, argc, initIndices, externs));
			code.push('}');
			code.push('');
		});
	}
...
function generateJNIProperty(options, metabase, state, code, indent, classname, classSig, propertyname, property, externs) {
	...
	if (options.platform == 'android' && !instance) {
		...
		if (found) {
			...
			log.fatal('property '+propertyname+' not found for class '+classname);
		}
	}
...
function generateJNIConstructor(options, metabase, state, code, indent, classname, classSig, constructors, argc, indices, externs) {
	code.push(indent+'Hyperloop::JNIEnv env;');
	...
	var condstart = true;
	indices[argc].forEach(function(i) {
		var method = constructors[i];
			cleanup = [],
			args = [];

		code.push(indent+'// '+method.signature);
		...
		if (method.args.length > 0) {
			var condstr = condition.join(' && ');
			if (condstart) {
				code.push(indent+'if ('+condstr+')');
				condstart = false;
			} else {
				code.push(indent+'else if ('+condstr+')');
			}
		}
		code.push(indent+'{');
	...
	code.push(indent+'}');
	code.push(indent+'return instance;');
}
...
// collect method indices order by argument count
function getMethodIndicesWithSameArgumentCount(methods) {
	var mIndices = {};
	if (Array.isArray(methods)) {
		methods.forEach(function(m,i){
			var argC = m.args.length;
			if (!mIndices[argC]) {
				mIndices[argC] = [];
			}
			mIndices[argC].push(i);
		});
	}
	else {
		mIndices[methods.args.length]=[];
	}
	return mIndices;
}
/**
...
function generateNewInstance(state, metabase, indent, classname, cast, name) {
	var typeobj = metabase.classes[classname],
		methodname = '<init>',
		methods = typeobj.methods[methodname],
		code = [];
	...
	var returnBlock = 'return JSValueMakeUndefined(ctx);';

	// collect method indices order by argument count
	var mIndices = getMethodIndicesWithSameArgumentCount(methods);

	code.push(indent+'switch(argumentCount)');
	code.push(indent+'{');
	Object.keys(mIndices).forEach(function(argc) {
		var mangled = jsgen.generateMethodName(classname, 'constructor_'+argc);
		code.push(indent+'\tcase ' + argc + ': instance = '+mangled+'(ctx, arguments, exception); break;');
	});
	code.push(indent+'}');
