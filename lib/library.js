...
function generateCustomJavaClass(options, state, metabase, fullClassname) {
	...
	if (options.platform == 'android') {
		code.push('import android.util.Log;');
		code.push('import android.app.Activity;');
		code.push('');
	}
	...
	if (options.platform == 'android') {
		var appname = options.appid+'.'+options.name+'Activity';
		// if we're going to overwrite main Activity on Android, we need to load additional libraries
		if (packagename+'.'+classname == appname) {
			code.push('\tprivate static final String TAG = "'+appname+'";');
		    code.push('\tstatic {');
		    code.push('\t\tLog.d(TAG,"Starting '+appname+'");');
		    code.push('\t\tSystem.loadLibrary("gnustl_shared");');
		    code.push('\t\tSystem.loadLibrary("JavaScriptCore");');
		    code.push('\t\tSystem.loadLibrary("'+options.name+'");');
		    code.push('\t\torg.appcelerator.hyperloop.Hyperloop.loadApp();');
		    code.push('\t}');
		}
	}
	...
	Object.keys(classinfo.methods).forEach(function(name) {
		classinfo.methods[name].forEach(function(method, i) {
			...
			if (method.shouldCallSuper) {
				code.push(indent+'\tsuper.'+name+'('+argv.join(',')+');');
			}
