...
function isStringPrimitive(node) {
	return node && node.start && node.start.type == 'string' && node.value;
}

function typeMatches(definedArg, calledArg) {
	var typeToMatch = definedArg.type;
	// TODO also look up assigned type for the calledArg in the state's symbol/node map?
	// I can't seem to use instanceof checks on the calledArg against Uglify AST node types
	// yuck, it seems to think it's just an Object
	switch(typeToMatch) {
		case 'boolean':
		case 'java.lang.Boolean': {
			// if arg is a boolean primitive, it's a match
			return calledArg && calledArg.start && calledArg.start.type == 'atom' && calledArg.start.value && (calledArg.start.value == 'true' || calledArg.start.value == 'false');
		}
		case 'java.lang.String': {
			// if arg is a String primitive, it's a match
			return isStringPrimitive(calledArg);
		}
		case 'char[]': {
			// if arg is an array of String primitives, it's a match
			if (calledArg && calledArg.elements) {
				return _.every(calledArg.elements, function(element){
					return isStringPrimitive(element);
				});
			}
			return false;
		}
		...
		case 'java.lang.Integer': {
			// if arg is a number primitive, it's a match
			// TODO warn the user if the number has decimal point that value will be coerced to int (dropping decimal)
			return calledArg && calledArg.start && calledArg.start.type == 'num' && calledArg.value && calledArg.value <= 2147483647 && calledArg.value >= -2147483648;
		}
		case 'short':
		case 'java.lang.Short': {
			// if arg is a number primitive, it's a match
			// TODO warn the user if the number has decimal point that value will be coerced to short (dropping decimal)
			return calledArg && calledArg.start && calledArg.start.type == 'num' && calledArg.value && calledArg.value <= 32767 && calledArg.value >= -32768;
		}
		case 'long':
		case 'java.lang.Long': {
			// if arg is a number primitive, it's a match
			// TODO warn the user if the number has decimal point that value will be coerced to long (dropping decimal)
			return calledArg && calledArg.start && calledArg.start.type == 'num' && calledArg.value && calledArg.value <= 9223372036854775807 && calledArg.value >= -9223372036854775808;
		}
	}
...
function findConstructor (metabase, cls, args, node, nodefail) {
	return findMethod(metabase, cls, "<init>", args, true, node, nodefail);
}
...
function getConstructorSymbol(state, cls, node, nodefail) {
	...
	log.debug("Found constructor: " + JSON.stringify(m));
	symbolname = jsgen.generateNewConstructorName(cls, "constructor") + library.mangleJavaSignature(m.signature);
	// generated call string based on args array
	var callstr =  m.name + '(';
	for (var i = 0, j = m.args.length; i < j; i++) {
		callstr += m.args[i];
		if (i != j - 1) {
	...
	return {type:'constructor',metatype:'constructor',symbolname:symbolname,class:cls,location:node.start,argcount:node.args.length,method:m,name:"<init>",selector:callstr,returnType:cls};	
}
