...
function getConstructorSymbol(state, cls, node, nodefail) {
	var m,
		symbolname;
	symbolname = jsgen.generateNewConstructorName(cls, "<init>");
	...
	// TODO Look up constructors based on arg counts and types!
	//m = findConstructor(state.metabase, cls, node.args, node, nodefail),
	//if (!m) {
	//	nodefail(node, "couldn't find constructor for class: "+cls.yellow+" with argcount "+node.args.length.toString().yellow);
	//}
	//symbolname = jsgen.generateNewConstructorName(cls, m.name);
	//return {type:'constructor',metatype:'constructor',symbolname:symbolname,class:cls,location:node.start,argcount:node.args.length,method:m};
}
